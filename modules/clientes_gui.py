...
class ClientesGUI:
    ...
    def setup_styles(self):
        ...
        style.configure("Title.TLabel", background=self.bg_color, foreground=self.primary_color, font=('Arial', 16, 'bold'))
        style.configure("Section.TLabel", background=self.bg_color, foreground=self.primary_color, font=('Arial', 12, 'bold'))
    ...
    def create_registro_tab(self, parent):
        ...
        self.entry_ruc = self.create_modern_entry(basic_frame, "RUC (11 dígitos):", 0, 0)
        self.entry_razon_social = self.create_modern_entry(basic_frame, "Razón Social:", 1, 0)
        self.entry_nombres = self.create_modern_entry(basic_frame, "Nombres:", 2, 0)
        self.entry_apellidos = self.create_modern_entry(basic_frame, "Apellidos:", 3, 0)
    ...
    def create_lista_tab(self, parent):
        ...
        frame_botones.grid(row=0, column=0, sticky="ew", pady=(0, 20))
        
        buttons = [
            ("Actualizar Lista", self.load_clientes, self.secondary_color),
            ("Editar Cliente", self.editar_cliente, self.warning_color),
            ("Ver Detalles", self.ver_detalles_cliente, "#2E86AB"),
            ("Eliminar Cliente", self.eliminar_cliente, self.accent_color)
        ]
    ...
    def editar_cliente(self):
        ...
        try:
            ...
            self.btn_guardar.config(text="Actualizar Cliente", bg=self.warning_color)
    ...
    def mostrar_detalles_cliente(self, cliente, direccion, telefonos, correos):
        ...
        tk.Label(frame_info, text=f"RUC: {cliente[0]}", 
                anchor="w", 
                font=("Arial", 10),
                bg=self.card_bg,
                fg=self.text_color).grid(row=0, column=0, sticky="w", padx=5, pady=2)
        tk.Label(frame_info, text=f"Razón Social: {cliente[1]}", 
                anchor="w", 
                font=("Arial", 10),
                bg=self.card_bg,
                fg=self.text_color).grid(row=1, column=0, sticky="w", padx=5, pady=2)
        tk.Label(frame_info, text=f"Nombres: {cliente[2]}", 
                anchor="w", 
                font=("Arial", 10),
                bg=self.card_bg,
                fg=self.text_color).grid(row=2, column=0, sticky="w", padx=5, pady=2)
        tk.Label(frame_info, text=f"Apellidos: {cliente[3]}", 
                anchor="w", 
                font=("Arial", 10),
                bg=self.card_bg,
                fg=self.text_color).grid(row=3, column=0, sticky="w", padx=5, pady=2)
    ...
    def cancelar_edicion(self):
        ...
        self.btn_guardar.config(text="Guardar Cliente", bg=self.success_color)
